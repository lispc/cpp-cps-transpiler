...
enum class __cps_min_treeEntryTag {
  Frame,
  Marker
};

struct __cps_min_treeEntry {
  __cps_min_treeEntryTag tag;
  __cps_min_treeFrame frame;
  int count;
  __cps_min_treeEntry(__cps_min_treeFrame f) : tag(__cps_min_treeEntryTag::Frame), frame(std::move(f)), count(0) {}
  __cps_min_treeEntry(int c, __cps_min_treeFrame f) : tag(__cps_min_treeEntryTag::Marker), frame(std::move(f)), count(c) {}
};
...
int min_tree(int n) {
  ...
  while (!__cps_stack.empty()) {
    ...
    if (__cps_entry.tag == __cps_min_treeEntryTag::Marker) {
