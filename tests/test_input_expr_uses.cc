// Adapted from ExprUsesParams in src/transformation_rules_helpers.cc.
// Walks a tree and returns true if any node carries the requested value.
...
bool expr_uses(Node *node, int target) {
  if (!node)
    return false;
  if (node->value == target)
    return true;
  for (const auto &child : node->children()) {
    if (expr_uses(child, target))
      return true;
  }
  return false;
}
