// Adapted from ContainsRecursiveCall in src/transformation_rules_helpers.cc.
// Walks a tree and returns true if any node is a call-node with the target id.
...
bool contains_rec(Node *node, int target_id) {
  if (!node)
    return false;
  if (node->kind == 1 && node->func_id == target_id)
    return true;
  for (const auto &child : node->children()) {
    if (contains_rec(child, target_id))
      return true;
  }
  return false;
}
